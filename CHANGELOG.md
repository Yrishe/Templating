...
### Added
- **Render deploy wiring** — [render.yaml](render.yaml) Blueprint provisions 6 services (backend web, frontend web, Celery worker, Celery beat, managed Postgres, managed Key Value/Redis) in one apply. Env vars wired via `fromDatabase` / `fromService` so the Postgres + Redis connection strings don't get copy-pasted. Secrets marked `sync: false` (Anthropic key, Sentry DSN, Brevo SMTP creds). Frontend Dockerfile gained `ARG NEXT_PUBLIC_*` so build-time env from Render bakes into the JS bundle. `cd-production.yml` rewritten: on `workflow_run` after CI passes on `main`, hits Render Deploy Hooks and polls `/api/docs/` for health. `backend-ci.yml` env mismatch fixed — Django reads `DB_*`, not `DATABASE_URL` / `POSTGRES_*`, so CI's Postgres service is now actually reachable. `.env.example` updated for Brevo SMTP. New [docs/deploy_runbook.md](docs/deploy_runbook.md) is the single-page ops reference (Brevo + Sentry + Cloudflare + first-manager bootstrap + smoke checklist + rollback).
