...
#### Your turn — concrete next steps

All the infra code is on the `deploy/render-blueprint` branch, but nothing goes live until the steps below happen. Each step is a click-through; no code changes are expected from here until the first deploy is verified.

1. **Review the PR** on GitHub (`deploy/render-blueprint`). Nothing merges to `main` until explicit approval — the CD workflow is gated on `main` pushes only, so this branch is safe to sit in review for as long as needed.
2. **Create accounts** (in parallel, ~20 min total):
   - **Brevo** — sign up, verify sending domain, generate an SMTP key.
   - **Sentry** — create a Django project, copy the DSN.
   - **Render** — install the GitHub app on `Yrishe/Templating` so Render can pull the repo.
3. **Apply the Blueprint** — Render dashboard → **New +** → **Blueprint** → select the repo → target the `deploy/render-blueprint` branch → approve. Render provisions 6 resources (backend, frontend, celery-worker, celery-beat, Postgres, Redis) in ~5–10 min.
4. **Fill the `sync: false` secrets** in each service's dashboard. See [docs/deploy_runbook.md §1.4](deploy_runbook.md) for the exact list: `ANTHROPIC_API_KEY`, `SENTRY_DSN`, `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD`, and optionally the AWS Textract creds. `DJANGO_SECRET_KEY` and `INBOUND_EMAIL_WEBHOOK_SECRET` are auto-generated by Render — copy the inbound secret for step 8.
5. **Bootstrap the first manager** via Render's backend Shell — `python manage.py createsuperuser` + promote to `role=MANAGER, is_active=True` as shown in [runbook §1.6](deploy_runbook.md). Without this, nobody can approve future manager signups.
6. **Smoke-test on the `*.onrender.com` URLs** before flipping DNS: log in, click through every feature in the [runbook §7 checklist](deploy_runbook.md), confirm no errors in Sentry. Iterate here if something breaks — it's cheaper than debugging through Cloudflare.
7. **Cloudflare DNS flip** — add two CNAMEs (`app` → frontend, `api` → backend), update `DJANGO_ALLOWED_HOSTS` + `CORS_ALLOWED_ORIGINS` on the backend, update `NEXT_PUBLIC_API_URL` + `NEXT_PUBLIC_WS_URL` on the frontend (requires a redeploy — the values are baked into the bundle at build time), verify WebSocket still works through the orange cloud ([runbook §4.3](deploy_runbook.md) covers the known gotcha).
8. **Brevo inbound parsing** — add an MX record pointing at Brevo's inbound server, configure Brevo's webhook to `https://api.<yourdomain>/api/webhooks/inbound-email/` with the `X-Webhook-Secret` header set to the value from step 4. End-to-end test: send an email to `<slug>@<inbound-domain>`, confirm an `IncomingEmail` row appears in Django admin.
9. **Wire up the GitHub secrets for CD** — paste each service's Deploy Hook URL into the repo's Actions secrets as `RENDER_DEPLOY_HOOK_BACKEND` / `RENDER_DEPLOY_HOOK_FRONTEND`, and set `PRODUCTION_URL` as an Actions variable ([runbook §1.5](deploy_runbook.md)).
10. **Merge the PR to `main`** — from this point on, every green main push auto-deploys to prod via the Deploy Hook, and worker + beat redeploy automatically because they share the backend image.

Flag any step that goes sideways — a failure at any numbered step usually means one specific thing (wrong secret, DNS not propagated, Cloudflare stripping WS headers) and the runbook has the diagnosis adjacent. Don't "redeploy and hope".
